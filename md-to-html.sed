...
# Currently using '.{0,100}[[:alnum:]\?!\)]+' in place of '.*'

# [![image](url)](url)
s/\[!\[ *(.{0,100}[[:alnum:]\?!\)]+) *\] *\( *([^ ]+) *\) *] *\( *([^ ]+) *\)/<a href="\3"><img src="\2" alt="\1"><\/a>/g

# ![image](url)
s/!\[ *(.{0,100}[[:alnum:]\?!\)]+) *\] *\( *([^ ]+) *\)/<img src="\2" alt="\1">/g

# [Web site](url)
s/\[ *(.{0,100}[[:alnum:]\?!\)]+) *\] *\( *([^ ]+) *"([^"]+)"\)/<a href="\2" title="\3">\1<\/a>/g
s/\[ *(.{0,100}[[:alnum:]\?!\)]+) *\] *\( *([^ ]+) *\)/<a href="\2">\1<\/a>/g
